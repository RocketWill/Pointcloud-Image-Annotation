...
import CanvasContextContextMenuComponent from 'components/annotation-page/canvas/canvas-context-context-menu';
import { updateCanvasContextContextMenu } from 'actions/annotation-actions';
...
function mapStateToProps(state: CombinedState): StateToProps {
    const {
        annotation: {
            ...
            canvas: {
                contextContextMenu: {
                    visible, top, left, contextIndex, type, clientID,
                },
                ready,
            },
            workspace,
...
function mapDispatchToProps(dispatch: ThunkDispatch): DispatchToProps {
    return {
        onStartIssue(position: number[]): void {
            ...
            dispatch(updateCanvasContextContextMenu(false, 0, 0, -1));
        },
        openIssue(position: number[], message: string): void {
            dispatch(reviewActions.startIssue(position));
...
class CanvasContextContextMenuContainer extends React.PureComponent<Props, State> {
    static defaultProps = {
        readonly: false,
    };
    ...
    public render(): JSX.Element {
        const { left, top, contextIndex } = this.state;
        const {
            visible,
            contextMenuClientID,
            objectStates,
        ...
        return (
            <>
                {type === ContextMenuType.CANVAS_SHAPE && (
                    <CanvasContextContextMenuComponent
...
export default connect(mapStateToProps, mapDispatchToProps)(CanvasContextContextMenuContainer);
